...
def write_DISCON(turbine, controller, param_file='DISCON.IN', txt_filename='Cp_Ct_Cq.txt', rosco_vt = {}):
    ...
    file.write('{0:<12d}        ! Twr_ControlMode   - Tower control mode {{0: no fore-aft control, 1: Frequency exclusion zone, 2: Tower fore-aft damping}}\n'.format(int(rosco_vt['Y_ControlMode'])))
    ...
    file.write('{:<015.5f}     ! VS_KP             - Proportional gain for generator PI torque controller [-]. (Only used in the transitional 2.5 region if VS_ControlMode =/ 2)\n'.format(rosco_vt['VS_KP']))
    file.write('{:<015.5f}     ! VS_KI             - Integral gain for generator PI torque controller [s]. (Only used in the transitional 2.5 region if VS_ControlMode =/ 2)\n'.format(rosco_vt['VS_KI']))
    ...
    file.write('{:<13.5f}       ! Twr_ExclBand	    - One-half of the total frequency exclusion band. Torque controller reference will be Twr_ExclFreq +/- Twr_ExlBand [rad/s]\n'.format(rosco_vt['Twr_ExclBand'] ))
