...
def write_DISCON(turbine, controller, param_file='DISCON.IN', txt_filename='Cp_Ct_Cq.txt', rosco_vt = {}):
    ...
    file.write('{}              ! PRC_WindSpeeds	- Array of wind speeds used in rotor speed vs. wind speed lookup table [m/s].\n'.format(''.join('{:<4.3f}  '.format(rosco_vt['PRC_WindSpeeds'][i]) for i in range(rosco_vt['PRC_n']))))            
    file.write('{}              ! PRC_RotorSpeeds	- Array of rotor speeds corresponding to PRC_WindSpeeds [rad/s].\n'.format(''.join('{:<4.3f}  '.format(rosco_vt['PRC_RotorSpeeds'][i]) for i in range(rosco_vt['PRC_n']))))            
    file.write('\n')
