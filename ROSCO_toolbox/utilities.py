...
def write_DISCON(turbine, controller, param_file='DISCON.IN', txt_filename='Cp_Ct_Cq.txt', rosco_vt = {}):
    ...
    file.write('{}                ! AWC_n              - AWC azimuthal number, must be an array the size of AWC_NumModes\n'.format(write_array(rosco_vt['AWC_n'],'<4d')))
    file.write('{}                ! AWC_omega          - AWC frequency, must be an array the size of AWC_NumModes [rad/s]\n'.format(write_array(rosco_vt['AWC_omega'],'<6.4f')))
    file.write('{}                ! AWC_amp            - AWC amplitude, must be an array the size of AWC_NumModes [rad]\n'.format(write_array(rosco_vt['AWC_amp'],'<6.4f')))
    file.write('{}                ! AWC_clockangle     - AWC clock angle, must be an array the size of AWC_NumModes [deg]\n'.format(write_array(rosco_vt['AWC_clockangle'],'<6.4f')))
    file.write('\n')
