...
!-------------------------------------------------------------------------------------------------------------------------------
    ...
    !       SS_Mode = 1, Implement setpoint smoothing
        ...
        IF ( CntrPar%SS_Mode == 1) THEN
            ...
            DelOmega = ((LocalVar%PC_PitComT - CntrPar%PC_MinPit)/0.524) * CntrPar%SS_VSGain - ((LocalVar%VS_RtPwr - LocalVar%VS_LastGenPwr))/CntrPar%VS_RtPwr * CntrPar%SS_PCGain ! Normalize to 30 degrees for now
