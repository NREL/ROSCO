...
Once the controller is compiled the turbine simulation tool needs to point to the compiled library.
...
It stores several flags and parameters needed by the controller and can be read by the compiled dynamic-link library.
Different turbines/tuning cases can be stored in separate input files but all of them could use a single dynamic-link library.
In OpenFAST, the :code:`DLL_InFile` parameter in the ServoDyn input file should be set to point to the desired input file.

The ROSCO toolbox is used to tune the ROSCO controller and generate the input file.
In order to tune the controller, ROSCO toolbox needs the OpenFAST model of the turbine and some user inputs in the form of a :code:`tuning.yaml` file.
