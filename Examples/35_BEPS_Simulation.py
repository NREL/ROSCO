...
As in AEPS, an eps_percent parameter is defined in this algorithm, and the predefined thrust limit is calculated based on the turbine’s maximum thrust using this parameter. The offset between the predefined thrust limit (ASO_ThrustLim) and the estimated turbine thrust (ASO_ThrustEst) is used to calculate an avoidance signal through a limit avoidance design parameter, e_dp. This avoidance signal is then applied to modify the blade pitch controller output, i.e., the blade pitch reference, collectively increasing the blade pitch angles when the turbine approaches the thrust limit. This avoidance action allows the turbine to operate safely within the defined thrust limit.
