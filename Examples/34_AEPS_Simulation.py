...
Finally, the predefined thrust force is calculated based on the maximum thrust force of the turbine using an eps_percent parameter. When the user selects an eps_percent value and activates the AEPS system (ASO = 1), based on the selected AEPS activation time ASO_StartTime for avoidance, the algorithm determines whether the turbine will operate under excessive thrust conditions and starts changing the blade pitch controller output, only when necessary, thereby adjusting the blade pitch angles and keeping the thrust force within the selected limit. AEPS is not recommended to be activated during learning phase, i.e at initial seconds or aggressive thrust behavior possibly occurring at the beginning of simulations.
...
•	wind margin (Um)
•	Effective limit avoidance parameter (ASO_ThrustGain)
AEPS tuning can be started with selecting the observer gain (Kc) and neural network (NN) learning rate (Γ) when AEPS is disabled (ASO = 0). The key purpose is to obtain an AEPS-predicted thrust force (Thrst_est) that is very close to the estimated turbine thrust force (ASO_ThrustEst). The designer can compare these two parameters (Thrst_est and ASO_ThrustEst). For ease of comparison, the designer can also check the error between them (T_error = ASO_ThrustEst – Thrst_est), which should be close to zero.
...
The learning rate Γ determines how quickly the neural network adapts to turbine operating conditions, i.e., predicting the turbine thrust force. A higher learning rate Γ results in faster learning but high oscillations in the AEPS-predicted thrust force. Once adaptation is achieved, the AEPS-predicted thrust (Thrst_est) should closely track the estimated turbine thrust force (ASO_ThrustEst). Here, it should be also noted that the adaptation/learning is satisfied for a range of these tuning values. Therefore, the designer should decide all of these values based on the satisfied error.
When the observer gain, learning rate and e-modification term gain are tuned as desired, AEPS can be activated (ASO = 1) to tune the effective limit avoidance parameter (ASO_ThrustGain). This parameter is highly sensitive to the turbine actuator. If the actuator model changes, ASO_ThrustGain may need to be re-tuned. It should be tuned such that the turbine thrust stays within the predefined thrust limit value. It should be kept in mind that choosing a high value for ASO_ThrustGain might cause oscillations in simulations. Also, the lag in the blade pitch response can also be compensated by the parameter Um, which helps AEPS generate an earlier avoidance signal in order to keep the turbine thrust within the limit. Um is just a wind margin showing how much close the wind speed estimate to the envelope wind speed.
Finally, if desired, after AEPS activation, the designer may fine-tune the observer gain and learning rate, again by comparing AEPS-predicted thrust with the estimated turbine thrust and monitoring the resulting error and also the limit avoidance parameter, ASO_ThrustGain , wind margin Um by checking the turbine thrust force whether staying within the limit or riding the turbine approximately at the limit. 
...
def main():
    ...
    r.wind_case_opts    = {
         'TMax': 300,  # from 10 to 15 m/s
         'wind_filenames': ['/Users/dzalkind/Downloads/testbench_NTM_U11.000000_Seed1501552846.0_DLC1.1_11ms_720s.bts'],
         }
