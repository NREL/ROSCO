...
AEPS tuning can be started with selecting the observer gain (Kc) and neural network (NN) learning rate (Γ) when AEPS is disabled (ASO = 0). The key purpose is to obtain an AEPS-predicted thrust force (ASO_ThrustNN) that is very close to the estimated turbine thrust force (ASO_ThrustEst). The designer can compare these two parameters (ASO_ThrustNN and ASO_ThrustEst). For ease of comparison, the designer can also check the error between them (T_error = ASO_ThrustEst – ASO_ThrustNN), which should be close to zero.
