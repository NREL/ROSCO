...
!-------------------------------------------------------------------------------------------------------------------------------
    ...
    ! Prefilter measured wind turbine signals to separate the filtering from the actual control actions
        ...
        IF (CntrPar%Fl_Mode > 0) THEN
            ! Force to start at 0
            LocalVar%NacIMU_FA_AccF = SecLPFilter(LocalVar%NacIMU_FA_Acc, LocalVar%DT, CntrPar%F_FlCornerFreq(1), CntrPar%F_FlCornerFreq(2), LocalVar%FP, LocalVar%iStatus, LocalVar%restart, objInst%instSecLPF, 0.0_DbKi) ! Fixed Damping
            LocalVar%FA_AccF = SecLPFilter(LocalVar%FA_Acc, LocalVar%DT, CntrPar%F_FlCornerFreq(1), CntrPar%F_FlCornerFreq(2), LocalVar%FP, LocalVar%iStatus, LocalVar%restart, objInst%instSecLPF, 0.0_DbKi) ! Fixed Damping
