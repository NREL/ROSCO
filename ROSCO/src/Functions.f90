...
!=======================================================================
    ...
    SUBROUTINE DEALLOCATE_ROSCO(CntrPar,PerfData)
        ! This function will deallocate ROSCO arrays so that it can be run multiple times
        !  A better solution would be to put all the data into a ROSCO_data array and deallocate that,
        ! but DZ was not able to make that work

        USE ROSCO_Types, ONLY : PerformanceData, ControlParameters
        IMPLICIT NONE
        TYPE(ControlParameters)         :: CntrPar
        TYPE(PerformanceData)           :: PerfData
    
        ! IF (ALLOCATED(RoscoVar)) THEN
        !     print *, 'Here'
        !     DEALLOCATE(RoscoVar)
        ! END IF
        ...
        IF (ALLOCATED(CntrPar%F_FlCornerFreq)) THEN
            DEALLOCATE(CntrPar%F_FlCornerFreq)
        END IF

        IF (ALLOCATED(CntrPar%F_FlpCornerFreq)) THEN
            DEALLOCATE(CntrPar%F_FlpCornerFreq)
        END IF
    
        IF (ALLOCATED(CntrPar%IPC_KI)) THEN
            DEALLOCATE(CntrPar%IPC_KI)
        END IF
    
        IF (ALLOCATED(CntrPar%IPC_aziOffset)) THEN
            DEALLOCATE(CntrPar%IPC_aziOffset)
        END IF
    
        IF (ALLOCATED(CntrPar%PC_GS_angles)) THEN
            DEALLOCATE(CntrPar%PC_GS_angles)
        END IF
    
    
        IF (ALLOCATED(CntrPar%PC_GS_KP)) THEN
            DEALLOCATE(CntrPar%PC_GS_KP)
        END IF
    
        IF (ALLOCATED(CntrPar%PC_GS_KI)) THEN
            DEALLOCATE(CntrPar%PC_GS_KI)
        END IF
    
    
        IF (ALLOCATED(CntrPar%PC_GS_KD)) THEN
            DEALLOCATE(CntrPar%PC_GS_KD)
        END IF
    
        IF (ALLOCATED(CntrPar%PC_GS_TF)) THEN
            DEALLOCATE(CntrPar%PC_GS_TF)
        END IF
    
        IF (ALLOCATED(CntrPar%VS_KP)) THEN
            DEALLOCATE(CntrPar%VS_KP)
        END IF
    
        IF (ALLOCATED(CntrPar%VS_KI)) THEN
            DEALLOCATE(CntrPar%VS_KI)
        END IF
    
        IF (ALLOCATED(CntrPar%WE_CP)) THEN
            DEALLOCATE(CntrPar%WE_CP)
        END IF
    
        IF (ALLOCATED(CntrPar%PerfTableSize)) THEN
            DEALLOCATE(CntrPar%PerfTableSize)
        END IF
    
        IF (ALLOCATED(CntrPar%WE_FOPoles_v)) THEN
            DEALLOCATE(CntrPar%WE_FOPoles_v)
        END IF
    
        IF (ALLOCATED(CntrPar%WE_FOPoles)) THEN
            DEALLOCATE(CntrPar%WE_FOPoles)
        END IF
    
        IF (ALLOCATED(CntrPar%Y_IPC_KP)) THEN
            DEALLOCATE(CntrPar%Y_IPC_KP)
        END IF
    
        IF (ALLOCATED(CntrPar%Y_IPC_KI)) THEN
            DEALLOCATE(CntrPar%Y_IPC_KI)
        END IF
    
        IF (ALLOCATED(CntrPar%PS_WindSpeeds)) THEN
            DEALLOCATE(CntrPar%PS_WindSpeeds)
        END IF
    
        IF (ALLOCATED(CntrPar%PS_BldPitchMin)) THEN
            DEALLOCATE(CntrPar%PS_BldPitchMin)
        END IF
    
        IF (ALLOCATED(PerfData%TSR_vec)) THEN
        ...
        IF (ALLOCATED(PerfData%Cp_mat)) THEN
            DEALLOCATE(PerfData%Cp_mat)
        END IF
    
        IF (ALLOCATED(PerfData%Ct_mat)) THEN
            DEALLOCATE(PerfData%Ct_mat)
        END IF
    
        IF (ALLOCATED(PerfData%Cq_mat)) THEN
    ...
    END SUBROUTINE DEALLOCATE_ROSCO
